...
DATA_DIR = ROOT / "local_tests" / "science_readiness" / "data" / "kpf_vega"
DATA_PATH = DATA_DIR / "KP.20250519.55029.51_L1.fits"
OUTPUT = ROOT / "local_tests" / "keck_kpf_vega_o2_comparison"
LINE_CACHE = (
    ROOT
    / "local_tests"
    / "science_readiness"
...
# Fixed from the established O2 bands and KPF order boundaries, before fitting.
SEGMENTS = (
    (13, 0.6868, 0.6905, "O2 B band"),
    (21, 0.7592, 0.7625, "O2 A band, blue order"),
    (22, 0.7630, 0.76515, "O2 A band, red order"),
)
FIT_RANGES_VACUUM_MICRON = tuple((lower, upper) for _, lower, upper, _ in SEGMENTS)
SPECIES = ("O2",)
KOAID = "KP.20250519.55029.51.fits"
KOA_FILEHANDLE = "/KPF/2025/20250519/lev2/KP.20250519.55029.51/KP.20250519.55029.51_L1.fits"
...
def _load_segments() -> tuple[list[Spectrum], fits.Header]:
    with fits.open(DATA_PATH, memmap=True) as hdul:
        ...
        wavelength_angstrom = np.asarray(hdul["RED_SCI_WAVE1"].data, dtype=float)
        flux = np.asarray(hdul["RED_SCI_FLUX1"].data, dtype=float)
        variance = np.asarray(hdul["RED_SCI_VAR1"].data, dtype=float)

    spectra: list[Spectrum] = []
    for order, lower, upper, label in SEGMENTS:
        wavelength = wavelength_angstrom[order] * 1.0e-4
        order_flux = flux[order]
        order_variance = variance[order]
        selected = (
            np.isfinite(wavelength)
            & np.isfinite(order_flux)
            & np.isfinite(order_variance)
            & (order_variance > 0)
            & (wavelength >= lower)
            & (wavelength <= upper)
        )
        if np.count_nonzero(selected) < 500:
            raise RuntimeError(f"Too few valid pixels in KPF order {order}")
        segment_wavelength = wavelength[selected]
        segment_flux = order_flux[selected]
        segment_uncertainty = np.sqrt(order_variance[selected])
        if segment_wavelength[0] > segment_wavelength[-1]:
            segment_wavelength = segment_wavelength[::-1]
            segment_flux = segment_flux[::-1]
            segment_uncertainty = segment_uncertainty[::-1]
        spectra.append(
            Spectrum(
...
def _run_pymolfit_systematics(
    ...
    seasonal_atmosphere = AtmosphereProfile.from_fits_header_mipas_gdas(
        header,
        gdas_mode="average",
        reference_wavenumber_cm=reference_wavenumber,
    )
    variants = {
        "seasonal_gdas": replace(baseline_config, atmosphere=seasonal_atmosphere),
        "continuum_order_2": replace(baseline_config, continuum_order=2),
        "continuum_order_4": replace(baseline_config, continuum_order=4),
    }
    started = time.perf_counter()
    ...
    result.write(OUTPUT / "systematics", prefix="kpf_segment")
...
def _run_pymolfit_cross_validation(
    ...
    elapsed = time.perf_counter() - started
    result.write(OUTPUT / "cross_validation", prefix="kpf_segment")
    return result, elapsed


def _sanitized_molecfit_header(source: fits.Header) -> fits.Header:
    date_beg = str(source.get("DATE-BEG", "2025-05-19T15:17:09.513"))
    utc_text = str(source.get("UTC", date_beg.split("T")[-1]))
    hours, minutes, seconds = (float(value) for value in utc_text.split(":"))
    utc_seconds = hours * 3600.0 + minutes * 60.0 + seconds
    header = fits.Header()
    header["OBJECT"] = str(source.get("OBJECT", "HR7001"))
    header["INSTRUME"] = "KPF"
    header["TELESCOP"] = "Keck I"
    header["OBSERVAT"] = "KECK"
    header["DATE-OBS"] = date_beg
    header["MJD-OBS"] = _header_float(source, "MJD-OBS", default=60814.636914)
    header["UTC"] = utc_seconds
    header["AIRMASS"] = _header_float(source, "AIRMASS", default=1.2)
    header["HIERARCH ESO TEL AIRM START"] = header["AIRMASS"]
    header["HIERARCH ESO TEL AIRM END"] = header["AIRMASS"]
    header["HIERARCH ESO TEL ALT"] = _header_float(source, "EL", default=56.62)
    header["HIERARCH ESO TEL GEOLAT"] = 19.82658656
    header["HIERARCH ESO TEL GEOLON"] = -155.4722
    ...
    ambient_temperature_c = _header_float(
        source,
        "AIRTEMP",
        "TEMP",
        "WXOUTTMP",
    ...
    header["HIERARCH ESO TEL AMBI TEMP"] = ambient_temperature_c
    header["HIERARCH ESO TEL TH M1 TEMP"] = ambient_temperature_c
    header["HIERARCH ESO INS SLIT1 WID"] = 1.14
...
def _write_molecfit_input(segments: list[Spectrum], header: fits.Header) -> Path:
    ...
    path = OUTPUT / "kpf_vega_o2_topocentric_vacuum.fits"
    fits.HDUList(
        [
...
def _run_molecfit(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="pymolfit_kpf_vega_") as temporary:
        ...
        _stage_molecfit_gdas_profile(atmosphere, staged_gdas)
        sof = stage / "model.sof"
        ...
        if completed.returncode != 0:
            raise RuntimeError(f"Molecfit KPF fit failed; see {OUTPUT / 'molecfit.log'}")
...
def _write_outputs(
    ...
    systematic_rms = np.concatenate(
        [segment.transmission_systematic_uncertainty for segment in systematics.segment_results]
    )
    systematic_envelope = np.concatenate(
        [segment.transmission_systematic_envelope for segment in systematics.segment_results]
    )
    systematic_corrected_uncertainty = np.concatenate(
        [segment.corrected.uncertainty for segment in systematics.segment_results]
    )
    ...
    systematic_rms = systematic_rms[:n_pixels]
    systematic_envelope = systematic_envelope[:n_pixels]
    systematic_corrected_uncertainty = systematic_corrected_uncertainty[:n_pixels]
    ...
    telluric = reliable & ((gen_transmission < 0.995) | (molecule["mtrans"] < 0.995))
    ...
    gdas_profile = Path(str(atmosphere.metadata.get("gdas_profile", "")))
    table.meta.update(
        {
            "source_koaid": KOAID,
            "source_filehandle": KOA_FILEHANDLE,
            "source_download_endpoint": "https://koa.ipac.caltech.edu/cgi-bin/KoaAPI/nph-dnloadL1data",
            "source_sha256": _sha256(DATA_PATH),
            "kpf_drp_tag": str(source_header.get("DRPTAG", "unknown")),
            "source_hdu": "RED_SCI_FLUX1/VAR1/WAVE1",
            "source_order_indices": json.dumps([entry[0] for entry in SEGMENTS]),
            ...
            "wavelength_frame": "topocentric vacuum",
            "gdas_source": str(atmosphere.metadata.get("gdas_source")),
            "gdas_before": str(atmosphere.metadata.get("gdas_before")),
            "gdas_after": str(atmosphere.metadata.get("gdas_after")),
            "gdas_profile_sha256": _sha256(gdas_profile) if gdas_profile.exists() else "",
            "systematics_variant_labels": json.dumps(list(systematics.variants)),
            "systematics_seasonal_gdas_source": str(
                seasonal_atmosphere.metadata.get("gdas_source")
            ),
            "observatory_site": str(atmosphere.metadata.get("observatory_site")),
    ...
    gen_objective = _weighted_objective(flux, gen_model, error)
    mol_objective = _weighted_objective(flux, molecule["mflux"], error)
    metrics = {
        ...
        "systematics_transmission_rms_p95": float(
            systematics.metrics["transmission_systematic_rms_p95"]
        ),
        "systematics_transmission_envelope_max": float(
            systematics.metrics["transmission_systematic_envelope_max"]
        ),
        "systematics_finite_fraction": float(
            systematics.metrics["finite_systematic_fraction"]
        ),
    ...
    fig, axes = plt.subplots(4, 3, figsize=(18, 11), sharex="col")
    ...
    start = 0
    for column, (_, lower, upper, label) in enumerate(SEGMENTS):
        ...
        axes[0, column].fill_between(
            x,
            gen_transmission[region] - systematic_envelope[region],
            gen_transmission[region] + systematic_envelope[region],
            color="C1",
            alpha=0.18,
            linewidth=0,
    ...
    fig.suptitle("Keck/KPF Vega: independent non-ESO O2 validation")
    fig.tight_layout()
    fig.savefig(OUTPUT / "keck_kpf_vega_o2_comparison.png", dpi=170)
    plt.close(fig)
    return metrics
...
def main() -> None:
    ...
    if not DATA_PATH.exists():
        raise FileNotFoundError(f"Missing KOA KPF validation product: {DATA_PATH}")
