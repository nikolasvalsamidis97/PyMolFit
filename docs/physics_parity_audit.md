# GenMolFit Physics Parity Audit
...
| Area | Reference source | GenMolFit implementation | Status |
| --- | --- | --- | --- |
| Physical constants | `lblrtm/src/phys_consts.f90` | `src/genmolfit/physics.py` | Defaults to modern Astropy constants. LBLRTM legacy constants are now exposed for parity tests. |
| Radiation term | `lblrtm/src/oprop_voigt.f90`, `RADFN` and `RADFNI` | `src/genmolfit/continuum.py`, `radiation_term_cm`, `radiation_term_interval_cm` | Matched for the pointwise three-branch formula and the interval bookkeeping used by panel accumulation. Tested with LBLRTM `RADCN2 = 1.4387752`. |
| HITRAN line strength temperature scaling | HITRAN formula; LBLRTM applies related correction in `oprop_voigt.f90` after LNFL preprocessing | `src/genmolfit/physics.py`, `line_strength_temperature`, `lblrtm_temperature_scaling_lower_energy` | Correct for HITRAN `.par` intensities with partition sums. LBLRTM unknown-EPP convention is now ported: values near `-1` skip temperature scaling and values below `-1.001` are made positive. Not bit-identical to LBLRTM TAPE3 internals because LNFL preprocesses intensity terms. |
| Pressure/Lorentz width | `lblrtm/src/oprop_voigt.f90` and `nonlte_voigt.f90`, `TMPCOR = TRATIO**TMPALF`; `lnfl/src/lnfl.f`, `BRDMATCH` | `src/genmolfit/physics.py`, `lorentz_hwhm_wavenumber`; `components.py`, broadener correction; `hitran.py`, AER parser | Matched for HITRAN exported temperature exponent convention: `(T_ref/T)**n`. Raw AER F100 records are now parsed with LNFL-style negative `IFLG` side rows, and the Molecfit-installed extra broadener files (`co2_h2o_brd_param`, `wv_co2_brd_param`, etc.) are auto-imported when present beside the AER database. O2/N2 HITRAN air-width self-mixture corrections from `RDLNFL` are ported. |
...
| Other continua/CIA | `contnm.f90` and HITRAN CIA tables | `continuum.py`, `TabulatedContinuum`, `HitranCIATable` | External-table supported. GenMolFit rejects source-continuum plus overlapping CIA combinations to prevent double counting. It does not yet bundle every UV or minor-species LBLRTM continuum model. |
...
- GenMolFit's public formula uses the HITRAN convention: `gamma(T) = gamma_ref * (T_ref/T)**n`.
...
formula behavior so future changes cannot silently move GenMolFit away from the
audited physics.
...
the relative-residual-scatter ratio is `0.9967`. GenMolFit's five nonlinear
parameters are full rank. This case did not supply any fitted Molecfit
coefficient to GenMolFit.
...
takes `64.999 s` on the same machine and input, so the current GenMolFit run is
...
the isolated 2017 HARPS Na D case, GenMolFit takes `2.33 s` versus Molecfit's
`29.72 s`.
